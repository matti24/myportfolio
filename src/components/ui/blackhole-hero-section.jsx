...
export function BlackHoleHeroSection({
  ...
  useEffect(() => {
    ...
    let visible = true;
    let raf = 0;
    // FPS-Deckel: die Temporal-Akkumulation ist langsam, 30fps sehen identisch aus und halbieren die GPU-Last
  ...
  return (
    <div
      ...
      {...rest}
    >
      {/* Statischer Fallback: sichtbar, falls WebGL fehlt (sonst deckt der opake Canvas ihn ab) */}
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
        <div
          className="absolute inset-0"
          style={{
            background:
              "radial-gradient(80% 65% at 72% 46%, rgba(255,140,50,0.12), rgba(120,50,10,0.06) 46%, #04050a 76%)",
          }}
        />
        {/* Weiche Andeutung der Akkretionsscheibe (gekippt, stark verwaschen) */}
        <div
          className="absolute left-[72%] top-[46%] h-[46vw] w-[46vw]"
          style={{
            transform: "translate(-50%, -50%) rotateX(76deg)",
            borderRadius: "9999px",
            background:
              "radial-gradient(closest-side, rgba(0,0,0,0) 44%, rgba(255,175,95,0.22) 52%, rgba(255,120,40,0.10) 64%, rgba(0,0,0,0) 82%)",
            filter: "blur(26px)",
          }}
        />
        {/* Dunkles Zentrum, weich eingeblendet */}
        <div
          className="absolute left-[72%] top-[46%] h-[16vw] w-[16vw]"
          style={{
            transform: "translate(-50%, -50%)",
            borderRadius: "9999px",
            background:
              "radial-gradient(closest-side, #03040a 55%, rgba(3,4,10,0.4) 80%, rgba(0,0,0,0) 100%)",
            filter: "blur(4px)",
          }}
        />
      </div>
